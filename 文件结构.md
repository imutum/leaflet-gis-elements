...
- **类型文件**: 自动复制 `src/index.d.ts` 到 `dist/`
...
查看 `src/js/controls/<控件名>/` 目录

### 要添加新样式？

1. 在 `src/js/styles/<控件名>/` 创建样式文件
2. 在控件初始化时注册样式

### 要修改控件外观？

查看 `src/css/controls/<控件名>-styles.css`

### 要调整全局配置？

查看 `src/js/constants.js`

### 要了解 API？

查看 `docs/TYPES.md` 或 `src/index.d.ts`
...
1. 在 `src/js/controls/` 创建目录
2. 继承 `BaseControl` 或 `StylableControl`
3. 在 `src/js/styles/` 创建样式
4. 在 `src/index.js` 导出
5. 在 `src/css/controls/` 添加样式（可选）
6. 更新 `docs/TYPES.md` 和 `src/index.d.ts`
7. 更新本文件
...
- **2024-10-30**: 添加 TypeScript 类型定义文件 `src/index.d.ts`
