...
class BaseMapControl {
     ...
     */
    _loadSavedPosition() {
        this.savedPosition = StorageUtils.load(this.storageKey);
    }
    ...
    _applyRelativePosition() {
        ...
        setTimeout(() => {
            if (this.container.parentNode && this.map) {
                ...
                const absolutePos = CoordinateUtils.relativeToAbsolute(
    ...
    _bindDragEvents() {
        ...
        const onMouseMove = (e) => {
            ...
            if (Math.abs(deltaX) > this.dragThreshold || Math.abs(deltaY) > this.dragThreshold) {
                ...
                const mapContainer = this.map.getContainer();
                const finalPosition = CoordinateUtils.constrainPosition(
        ...
        const onMouseUp = (e) => {
            if (this.isDragging && hasMoved) {
                ...
                const relativePos = CoordinateUtils.absoluteToRelative(
                    absolutePos.x, absolutePos.y, mapContainer
                );

                StorageUtils.save(this.storageKey, relativePos);
...
// 导出到全局
window.BaseMapControl = BaseMapControl;
