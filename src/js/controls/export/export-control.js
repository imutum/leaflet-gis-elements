...
class ExportControl extends L.GISElements.BaseControl {
    constructor(options = {}) {
        super({
    ...
    _showPreviewBorder() {
        ...
        // 使用常量获取配置
        const borderColor = L.GISElements.Constants ? L.GISElements.Constants.COLORS.PREVIEW_BORDER : '#ff6b6b';
        const zIndex = L.GISElements.Constants ? L.GISElements.Constants.Z_INDEX.EXPORT_PREVIEW : 500;
        ...
        if (L.GISElements.Draggable) {
            this.previewDraggableManager = new L.GISElements.Draggable(this.previewBorder, {
        ...
        if (L.GISElements.Resizable) {
            const minSize = L.GISElements.Constants ? L.GISElements.Constants.MIN_FRAME_WIDTH : 100;

            this.previewResizableManager = new L.GISElements.Resizable(this.previewBorder, {
                minWidth: minSize,
                minHeight: minSize,
...
// 导出到 L.Control 命名空间
L.Control.Export = ExportControl;

// 工厂方法
L.control.export = function (options) {
    const control = new L.Control.Export(options);
